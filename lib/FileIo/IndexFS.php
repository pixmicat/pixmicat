...
class IndexFS
{
    ...
    public function openIndex()
    {
        if (\extension_loaded('pdo_sqlite')) {
            ...
            $indexlog_count = \count($indexlog); // 讀入索引檔並計算目前筆數
            $this->index = array();
    ...
    /* 搜尋預覽圖檔檔名 */
    public function findThumbName($pattern)
    {
        switch ($this->backend) {
            case 'pdo_sqlite':
    ...
    /* 新增一筆索引 */
    public function addRecord($id, $imgSize, $imgURL)
    {
        switch ($this->backend) {
            case 'pdo_sqlite':
    ...
    public function saveIndex()
    {
        if ($this->backend=='log' && $this->modified) { // 如果有修改索引就回存
            $indexlog = '';
            if (\count($this->index)) {
                foreach ($this->index as $ikey => $ival) {
                    $indexlog .= $ikey."\t\t".$ival['imgSize']."\t\t".$ival['imgURL']."\n";
                }
            } // 有資料才跑迴圈
