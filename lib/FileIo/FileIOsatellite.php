...
class FileIOsatellite extends AbstractIfsFileIO
{
     ...
     * 傳送抓取要求到遠端衛星主機上面
     */
    private function transloadSatellite($imgname)
    {
        if (!($fp = \fsockopen($this->parameter[0]['host'], 80))) {
            return false;
        }
     ...
     * 直接傳送檔案到遠端衛星主機上面
     */
    private function uploadSatellite($imgname, $imgpath)
    {
     ...
     * 發出刪除圖片要求
     */
    private function deleteSatellite($imgname)
    {
        if (!($fp = \fsockopen($this->parameter[0]['host'], 80))) {
            return false;
        }
    ...
    public function uploadImage($imgname, $imgpath, $imgsize)
    {
        ...
        $result = $this->parameter[1]
                ? $this->transloadSatellite($imgname)
                : $this->uploadSatellite($imgname, $imgpath);
        if ($result) {
